...
import { getAllAds, tagAd, deleteAd, deleteAllUntagged } from '../state.js'
  ...
  setup(_, { emit }) {
    ...
    function refresh() {
      ...
      ads.value.forEach(ad => {
        if (tagInputs.value[ad.id] === undefined) {
          tagInputs.value[ad.id] = ad.tag || ''
        }
      })
    }
    refresh()
    ...
    const filtered = computed(() => {
      if (filter.value === 'tagged')   return ads.value.filter(a => a.tag && a.tag.trim())
      if (filter.value === 'untagged') return ads.value.filter(a => !a.tag || !a.tag.trim())
      return ads.value
    })
    ...
    function remove(id) {
      if (confirm('Delete this ad?')) {
        deleteAd(id)
        delete tagInputs.value[id]
        refresh()
      }
    }

    function bulkDelete() {
      const count = ads.value.filter(a => !a.tag || !a.tag.trim()).length
