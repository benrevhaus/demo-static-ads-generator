...
;(function() {
  ...
  window.AdGen.Step1Image = defineComponent({
    name: 'Step1Image',
    props: { data: Object },
    emits: ['update', 'next'],
    setup(props, { emit }) {
      const prompt    = reactive(Object.assign({}, props.data.imagePrompt))
      const imageRef  = ref(props.data.imageRef || '')
      const generating = ref(false)
      const error     = ref('')

      function builtPrompt() {
        return prompt.mood + ' ' + prompt.productCategory + ' ad — ' +
          (prompt.subject || 'product') + ' on ' + (prompt.backgroundStyle || 'clean') +
          ' background with ' + (prompt.colorHints || 'natural') + ' tones. Campaign: "' +
          (prompt.campaignHeader || 'Untitled') + '"'
      }
      ...
      async function generate() {
        ...
        try {
          ...
          const dataUrl = window.AdGen.generatePlaceholderImage(prompt, 900, 450)
          imageRef.value = dataUrl
          emit('update', { imagePrompt: Object.assign({}, prompt), imageRef: dataUrl })
        } catch (e) {
      ...
      function proceed() {
        ...
        emit('update', { imagePrompt: Object.assign({}, prompt), imageRef: imageRef.value })
        emit('next')
      }
